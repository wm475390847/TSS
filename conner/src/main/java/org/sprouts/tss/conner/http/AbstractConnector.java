...
import org.sprouts.tss.conner.Context;
...
public abstract class AbstractConnector implements IConnector<Response> {
    ...
    public ResponseLog<Response> getLog() {
        ...
        Optional.of(response).filter(Response::isSuccessful).orElseThrow(() -> new ConnerException(response.header("Requestid"), response.toString()));
