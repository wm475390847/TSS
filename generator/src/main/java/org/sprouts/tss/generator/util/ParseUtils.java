...
import org.sprouts.tss.generator.enums.KeywordEnum;
...
public class ParseUtils {
    ...
    public static String getClassName(String apiPath, String apiMethod, String apiSuffix) {
        ...
        for (int i = index; i < length; i++) {
            sb.append(StringUtils.firstUpperCase(pathList[i]));
        }
        String name = StringUtils.firstUpperCase(sb.toString());
        if (apiMethod != null) {
            name = name + StringUtils.lineToHump(apiMethod.toLowerCase(), true);
        }
    ...
    private static String[] getPathList(String apiPath) {
        ...
        return Arrays.stream(pathList)
                ...
                .map(e -> StringUtils.lineToHump(e, true))
                .toArray(String[]::new);
    }
}
